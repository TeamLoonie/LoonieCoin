...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 3734, 14315));
    ...
    strUsage += HelpMessageOpt("-zmqpubhashtxlock=<address>", _("Enable publish hash transaction (locked via BriskTX) in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawblock=<address>", _("Enable publish raw block in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtx=<address>", _("Enable publish raw transaction in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtxlock=<address>", _("Enable publish raw transaction (locked via BriskTX) in <address>"));
#endif
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, loonie, (purification, brisktx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all LOONIE specific functionality (Masternodes, Purification, BriskTX) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:3734"));
    strUsage += HelpMessageOpt("-budgetvotemode=<mode>", _("Change automatic finalized budget voting behavior. mode=auto: Vote for only exact finalized budget match to my generated budget. (string, default: auto)"));

    strUsage += HelpMessageGroup(_("Purification options:"));
    strUsage += HelpMessageOpt("-enablepurification=<n>", strprintf(_("Enable use of automated purification for funds stored in this wallet (0-1, default: %u)"), 0));
    strUsage += HelpMessageOpt("-purificationrounds=<n>", strprintf(_("Use N separate masternodes to anonymize funds  (2-8, default: %u)"), 2));
    strUsage += HelpMessageOpt("-anonymizeloonieamount=<n>", strprintf(_("Keep N LNI anonymized (default: %u)"), 0));
    strUsage += HelpMessageOpt("-liquidityprovider=<n>", strprintf(_("Provide liquidity to Purification by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0));

    strUsage += HelpMessageGroup(_("BriskTX options:"));
    strUsage += HelpMessageOpt("-enablebrisktx=<n>", strprintf(_("Enable briskTX, show confirmations for locked transactions (bool, default: %s)"), "true"));
    strUsage += HelpMessageOpt("-brisktxdepth=<n>", strprintf(_("Show N confirmations for a successfully locked transaction (0-9999, default: %u)"), nBriskTXDepth));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph("Copyright (C) 2018 LOONIE") + "\n" +
           "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!GetBoolArg("-enablebrisktx", fEnableBriskTX)) {
        if (SoftSetArg("-brisktxdepth", 0))
            LogPrintf("AppInit2 : parameter interaction: -enablebrisktx=false -> setting -nBriskTXDepth=0\n");
    }
...
// ********************************************************* Step 8: load wallet
    ...
    fEnablePurification = GetBoolArg("-enablepurification", false);

    nPurificationRounds = GetArg("-purificationrounds", 2);
    if (nPurificationRounds > 16) nPurificationRounds = 16;
    if (nPurificationRounds < 1) nPurificationRounds = 1;

    nLiquidityProvider = GetArg("-liquidityprovider", 0); //0-100
    if (nLiquidityProvider != 0) {
        obfuScationPool.SetMinBlockSpacing(std::min(nLiquidityProvider, 100) * 15);
        fEnablePurification = true;
        nPurificationRounds = 99999;
    }
    ...
    fEnableBriskTX = GetBoolArg("-enablebrisktx", fEnableBriskTX);
    nBriskTXDepth = GetArg("-brisktxdepth", nBriskTXDepth);
    nBriskTXDepth = std::min(std::max(nBriskTXDepth, 0), 60);

    //lite mode disables all Masternode and Purification related functionality
    ...
    LogPrintf("nBriskTXDepth %d\n", nBriskTXDepth);
    LogPrintf("Purification rounds %d\n", nPurificationRounds);
    LogPrintf("Anonymize LOONIE Amount %d\n", nAnonymizeLOONIEAmount);
    LogPrintf("Budget Mode %s\n", strBudgetMode.c_str());

    /* Denominations

       A note about convertability. Within Purification pools, each denomination
