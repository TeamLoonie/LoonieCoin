...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
...

    {
        ...
        if (mapPurificationBroadcastTxes.count(hash)) {
...
bool AcceptableInputs(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool isLNIX)
{
    AssertLockHeld(cs_main);
    if (pfMissingInputs)
        *pfMissingInputs = false;
...
int64_t GetBlockValue(int nHeight)
{
    ...
    // Check if we reached the coin max supply.Yep,as it says we do have a way to cap coins at max supply,finally.
...
/*
    ...
    Remove conflicting blocks for successful BriskTX transaction locks
...
bool static AlreadyHave(const CInv& inv)
{
    ...
    case MSG_LNIX:
        return mapPurificationBroadcastTxes.count(inv.hash);
    case MSG_BLOCK:
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv& inv = *it;
        {
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK) {
                ...
                if (!pushed && inv.type == MSG_LNIX) {
                    if (mapPurificationBroadcastTxes.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mapPurificationBroadcastTxes[inv.hash].tx << mapPurificationBroadcastTxes[inv.hash].vin << mapPurificationBroadcastTxes[inv.hash].vchSig << mapPurificationBroadcastTxes[inv.hash].sigTime;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        if (strCommand == "tx") {
            ...
            if (pmn != NULL) {
                ...
                if (!mapPurificationBroadcastTxes.count(tx.GetHash())) {
                    ...
                    mapPurificationBroadcastTxes.insert(make_pair(tx.GetHash(), dstx));
                }
            }
        }
    ...
    // getaddr message mitigates the attack.
        ...
        obfuScationPool.ProcessMessagePurification(pfrom, strCommand, vRecv);
        mnodeman.ProcessMessage(pfrom, strCommand, vRecv);
        budget.ProcessMessage(pfrom, strCommand, vRecv);
        masternodePayments.ProcessMessageMasternodePayments(pfrom, strCommand, vRecv);
        ProcessMessageBriskTX(pfrom, strCommand, vRecv);
