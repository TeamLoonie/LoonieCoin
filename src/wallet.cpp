...
// Recursively determine the rounds of a given input (How deep is the Purification chain for a given input)
int CWallet::GetRealInputPurificationRounds(CTxIn in, int rounds) const
{
    ...
    if (wtx != NULL) {
        ...
        if (mdwi == mDenomWtxes.end()) {
            LogPrint("purification", "GetInputPurificationRounds INSERTING %s\n", hash.ToString());
        ...
        if (nout >= wtx->vout.size()) {
            ...
            LogPrint("purification", "GetInputPurificationRounds UPDATED   %s %3d %3d\n", hash.ToString(), nout, -4);
            return -4;
        }
        ...
        BOOST_FOREACH (CTxIn in2, wtx->vin) {
            if (IsMine(in2)) {
                int n = GetRealInputPurificationRounds(in2, rounds + 1);
...
int CWallet::GetInputPurificationRounds(CTxIn in) const
{
    LOCK(cs_wallet);
    int realPurificationRounds = GetRealInputPurificationRounds(in, 0);
    return realPurificationRounds > nPurificationRounds ? nPurificationRounds : realPurificationRounds;
}
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                int rounds = GetInputPurificationRounds(vin);
                fTotal += (float)rounds;
                fCount += 1;
            }
        }
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                int rounds = GetInputPurificationRounds(vin);
                nTotal += pcoin->vout[i].nValue * rounds / nPurificationRounds;
            }
        }
    }
    ...
    if (coinControl && coinControl->HasSelected()) {
        BOOST_FOREACH (const COutput& out, vCoins) {
            ...
            if (coin_type == ONLY_DENOMINATED) {
                ...
                if (rounds < nPurificationRounds) continue;
            }
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, CAmount& nValueRet, int nPurificationRoundsMin, int nPurificationRoundsMax)
{
    vCoinsRet.clear();
    nValueRet = 0;
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (nValueRet + out.tx->vout[out.i].nValue <= nValueMax) {
            ...
            int rounds = GetInputPurificationRounds(vin);
            if (rounds >= nPurificationRoundsMax) continue;
            if (rounds < nPurificationRoundsMin) continue;
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& setCoinsRet, CAmount& nValueRet, int nPurificationRoundsMin, int nPurificationRoundsMax) const
{
    ...
    vector<COutput> vCoins;
    AvailableCoins(vCoins, true, coinControl, nPurificationRoundsMin < 0 ? ONLY_NONDENOMINATED_NOT10000IFMN : ONLY_DENOMINATED);
...
bool CWallet::CreateCollateralTransaction(CMutableTransaction& txCollateral, std::string& strReason)
{
    ...
    */
    ...
    if (!SelectCoinsCollateral(vCoinsCollateral, nValueIn2)) {
        strReason = "Error: Purification requires a collateral transaction and could not locate an acceptable input!";
        return false;
    }
    ...
    BOOST_FOREACH (CTxIn v, txCollateral.vin) {
        if (!SignSignature(*this, v.prevPubKey, txCollateral, vinNumber, int(SIGHASH_ALL | SIGHASH_ANYONECANPAY))) {
            ...
            strReason = "CPurificationPool::Sign - Unable to sign collateral transaction! \n";
            return false;
        }
        vinNumber++;
    }
...
{
    LogPrintf("CWallet::GetBudgetSystemCollateralTX - Budget feature has been disabled on LOONIE network\n");
    return false;
}
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 30000 LNI.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Purification non-denominated funds for this transaction that are not equal 30000 LNI.");
                    } else {
                        strFailReason = _("Unable to locate enough Purification denominated funds for this transaction.");
                        strFailReason += " " + _("Purification uses exact denominated amounts to send funds, you might simply need to anonymize some more coins.");
                    }

                    if (useIX) {
                        strFailReason += " " + _("BriskTX requires inputs with at least 6 confirmations, you might need to wait a few minutes and try again.");
                    }
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew, unsigned int& nTxNewTime)
{
    ...
    BOOST_FOREACH (PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setStakeCoins) {
        ...
        if (CheckStakeKernelHash(nBits, block, *pcoin.first, prevoutStake, nTxNewTime, nHashDrift, false, hashProofOfStake, true)) {
            ...
            if (nTotalSize / 2 > nStakeSplitThreshold * COIN && nTotalSize / 2 > GetMasternodePayment(pIndex0->nHeight, GetBlockValue(pIndex0->nHeight)))
...
string CWallet::PreparePurificationDenominate(int minRounds, int maxRounds)
{
    if (IsLocked())
    ...
    if (obfuScationPool.GetState() != POOL_STATUS_ERROR && obfuScationPool.GetState() != POOL_STATUS_SUCCESS)
        if (obfuScationPool.GetEntriesCount() > 0)
            return _("Error: You already have pending entries in the Purification pool");
    ...
    */
    ...
    LogPrintf("PreparePurificationDenominate - preparing purification denominate . Got: %d \n", nValueIn);
    ...
    */
    ...
    obfuScationPool.SendPurificationDenominate(vCoinsResult, vOut, nValueIn - nValueLeft);
