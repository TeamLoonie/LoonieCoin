...
#include "purification-relay.h"
#include "sync.h"

class CTxIn;
class CPurificationPool;
class CObfuScationSigner;
class CMasterNodeVote;
class CBitcoinAddress;
class CPurificationQueue;
class CPurificationBroadcastTx;
...
extern CPurificationPool obfuScationPool;
extern CObfuScationSigner obfuScationSigner;
extern std::vector<CPurificationQueue> vecPurificationQueue;
extern std::string strMasterNodePrivKey;
extern map<uint256, CPurificationBroadcastTx> mapPurificationBroadcastTxes;
extern CActiveMasternode activeMasternode;

/** Holds an Purification input
 */
...
// A clients transaction in the purification pool
...
public:
    ...
    /// Add entries to use for Purification
...
/**
 * A currently inprogress Purification merge and denomination information
 */
class CPurificationQueue
{
public:
    ...
    /** Sign this Purification transaction
...
/** Helper class to store Purification transaction (tx) information.
 */
class CPurificationBroadcastTx
{
public:
...
/** Used to keep track of current status of Purification pool
 */
class CPurificationPool
{
private:
    mutable CCriticalSection cs_purification;
...
public:
    ...
    CPurificationPool()
    {
        /* Purification uses collateral addresses to trust parties entering the pool
    ...
    /** Process a Purification message using the Purification protocol
     * \param pfrom
     * \param strCommand lower case command string; valid values are:
     *        Command  | Description
     *        -------- | -----------------
     *        dsa      | Purification Acceptable
     *        dsc      | Purification Complete
     *        dsf      | Purification Final tx
     *        dsi      | Purification vIn
     *        dsq      | Purification Queue
     *        dss      | Purification Signal Final Tx
     *        dssu     | Purification status update
     *        dssub    | Purification Subscribe To
     * \param vRecv
     */
    void ProcessMessagePurification(CNode* pfrom, std::string& strCommand, CDataStream& vRecv);

    void InitCollateralAddress()
    {
        SetCollateralAddress(Params().PurificationPoolDummyAddress());
    }
    ...
    void UpdateState(unsigned int newState)
    {
        if (fMasterNode && (newState == POOL_STATUS_ERROR || newState == POOL_STATUS_SUCCESS)) {
            LogPrint("purification", "CPurificationPool::UpdateState() - Can't set state to ERROR or SUCCESS as a Masternode. \n");
            return;
        }

        LogPrintf("CPurificationPool::UpdateState() == %d | %d \n", state, newState);
        if (state != newState) {
    ...
    /// Passively run Purification in the background according to the configuration in settings (only for QT)
    bool DoAutomaticDenominating(bool fDryRun = false);
    bool PreparePurificationDenominate();

    /// Check for process in Purification
    void Check();
    ...
    void SendPurificationDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, CAmount amount);
    /// Get Masternode updates about the progress of Purification
