...
// The main object for accessing Purification
CPurificationPool obfuScationPool;
// A helper object for signing messages from Masternodes
CObfuScationSigner obfuScationSigner;
// The current Purifications in progress on the network
std::vector<CPurificationQueue> vecPurificationQueue;
// Keep track of the used Masternodes
std::vector<CTxIn> vecMasternodesUsed;
// Keep track of the scanning errors I've seen
map<uint256, CPurificationBroadcastTx> mapPurificationBroadcastTxes;
...
void CPurificationPool::ProcessMessagePurification(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    if (fLiteMode) return; //disable all Purification/Masternode related functionality
    if (!masternodeSync.IsBlockchainSynced()) return;

    if (strCommand == "dsa") { //Purification Accept Into Pool
    ...
    } else if (strCommand == "dsq") { //Purification Queue
        TRY_LOCK(cs_purification, lockRecv);
        if (!lockRecv) return;
        ...
        if (dsq.ready) {
            ...
            if (state == POOL_STATUS_QUEUE) {
                LogPrint("purification", "Purification queue is ready - %s\n", addr.ToString());
                PreparePurificationDenominate();
            }
        } else {
            BOOST_FOREACH (CPurificationQueue q, vecPurificationQueue) {
                if (q.vin == dsq.vin) return;
            }

            LogPrint("purification", "dsq last %d last2 %d count %d\n", pmn->nLastDsq, pmn->nLastDsq + mnodeman.size() / 5, mnodeman.nDsqCount);
            //don't allow a few nodes to dominate the queuing process
            if (pmn->nLastDsq != 0 &&
                pmn->nLastDsq + mnodeman.CountEnabled(ActiveProtocol()) / 5 > mnodeman.nDsqCount) {
                LogPrint("purification", "dsq -- Masternode sending too many dsq messages. %s \n", pmn->addr.ToString());
                return;
            }
            mnodeman.nDsqCount++;
            ...
            LogPrint("purification", "dsq - new Purification queue object - %s\n", addr.ToString());
            vecPurificationQueue.push_back(dsq);
            dsq.Relay();
            dsq.time = GetTime();
        }
        ...
        //check it like a transaction
        {
            ...
            BOOST_FOREACH (const CTxIn i, in) {
                ...
                LogPrint("purification", "dsi -- tx in %s\n", i.ToString());
            ...
            if (nValueIn > OBFUSCATION_POOL_MAX) {
                LogPrintf("dsi -- more than Purification pool max! %s\n", tx.ToString());
                errorID = ERR_MAXIMUM;
                pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), MASTERNODE_REJECTED, errorID);
                return;
            }
    ...
    } else if (strCommand == "dssu") { //Purification status update
        if (pfrom->nVersion < ActiveProtocol()) {
            return;
        }
        ...
        LogPrint("purification", "dssu - state: %i entriesCount: %i accepted: %i error: %s \n", state, entriesCount, accepted, GetMessageByID(errorID));

        if ((accepted != 1 && accepted != 0) && sessionID != sessionIDMessage) {
            LogPrintf("dssu - message doesn't match current Purification session %d %d\n", sessionID, sessionIDMessage);
            return;
        }

        StatusUpdate(state, entriesCount, accepted, errorID, sessionIDMessage);

    } else if (strCommand == "dss") { //Purification Sign Final Tx
        ...
        BOOST_FOREACH (const CTxIn item, sigs) {
            ...
            LogPrint("purification", " -- sigs count %d %d\n", (int)sigs.size(), count);
            count++;
        }
    ...
    } else if (strCommand == "dsf") { //Purification Final tx
        if (pfrom->nVersion < ActiveProtocol()) {
            return;
        }
        ...
        if (sessionID != sessionIDMessage) {
            LogPrint("purification", "dsf - message doesn't match current Purification session %d %d\n", sessionID, sessionIDMessage);
            return;
        }
    ...
    } else if (strCommand == "dsc") { //Purification Complete
        ...
        if (sessionID != sessionIDMessage) {
            LogPrint("purification", "dsc - message doesn't match current Purification session %d %d\n", obfuScationPool.sessionID, sessionIDMessage);
            return;
        }
...
void CPurificationPool::SetNull()
{
    // MN side
    sessionUsers = 0;
...
bool CPurificationPool::SetCollateralAddress(std::string strAddress)
{
    CBitcoinAddress address;
    if (!address.SetString(strAddress)) {
        LogPrintf("CPurificationPool::SetCollateralAddress - Invalid Purification collateral address\n");
        return false;
    }
...
//
// Unlock coins after Purification fails or succeeds
//
void CPurificationPool::UnlockCoins()
{
    while (true) {
...
std::string CPurificationPool::GetStatus()
{
    ...
    case POOL_STATUS_ACCEPTING_ENTRIES:
        if (entriesCount == 0) {
            ...
            return _("Purification request complete:") + " " + _("Your transaction was accepted into the pool!");
        } else {
            std::string suffix = "";
    ...
    case POOL_STATUS_ERROR:
        return _("Purification request incomplete:") + " " + lastMessage + " " + _("Will retry...");
    case POOL_STATUS_SUCCESS:
        return _("Purification request complete:") + " " + lastMessage;
...
//
// Check the Purification progress and send client updates if a Masternode
//
void CPurificationPool::Check()
{
    if (fMasterNode) LogPrint("purification", "CPurificationPool::Check() - entries count %lu\n", entries.size());
    //printf("CPurificationPool::Check() %d - %d - %d\n", state, anonTx.CountEntries(), GetTimeMillis()-lastTimeChanged);

    if (fMasterNode) {
        LogPrint("purification", "CPurificationPool::Check() - entries count %lu\n", entries.size());

        // If entries is full, then move on to the next phase
        if (state == POOL_STATUS_ACCEPTING_ENTRIES && (int)entries.size() >= GetMaxPoolTransactions()) {
            LogPrint("purification", "CPurificationPool::Check() -- TRYING TRANSACTION \n");
    ...
    if (state == POOL_STATUS_FINALIZE_TRANSACTION) {
        LogPrint("purification", "CPurificationPool::Check() -- FINALIZE TRANSACTIONS\n");
        ...
        if (fMasterNode) {
            ...
            LogPrint("purification", "Transaction 1: %s\n", txNew.ToString());
    ...
    if (fMasterNode && state == POOL_STATUS_SIGNING && SignaturesComplete()) {
        LogPrint("purification", "CPurificationPool::Check() -- SIGNING\n");
    ...
    if ((state == POOL_STATUS_ERROR || state == POOL_STATUS_SUCCESS) && GetTimeMillis() - lastTimeChanged >= 10000) {
        LogPrint("purification", "CPurificationPool::Check() -- timeout, RESETTING\n");
        UnlockCoins();
        SetNull();
...
void CPurificationPool::CheckFinalTransaction()
{
    ...
    {
        LogPrint("purification", "Transaction 2: %s\n", txNew.ToString());

        // See if the transaction is valid
        if (!txNew.AcceptToMemoryPool(false, true, true)) {
            LogPrintf("CPurificationPool::Check() - CommitTransaction : Error: Transaction not valid\n");
            SetNull();
        ...
        LogPrintf("CPurificationPool::Check() -- IS MASTER -- TRANSMITTING OBFUSCATION\n");
        ...
        if (!obfuScationSigner.SetKey(strMasterNodePrivKey, strError, key2, pubkey2)) {
            LogPrintf("CPurificationPool::Check() - ERROR: Invalid Masternodeprivkey: '%s'\n", strError);
            return;
        }

        if (!obfuScationSigner.SignMessage(strMessage, strError, vchSig, key2)) {
            LogPrintf("CPurificationPool::Check() - Sign message failed\n");
            return;
        }

        if (!obfuScationSigner.VerifyMessage(pubkey2, vchSig, strMessage, strError)) {
            LogPrintf("CPurificationPool::Check() - Verify message failed\n");
            return;
        }

        if (!mapPurificationBroadcastTxes.count(txNew.GetHash())) {
            CPurificationBroadcastTx dstx;
            dstx.tx = txNew;
            dstx.vin = activeMasternode.vin;
            dstx.vchSig = vchSig;
            dstx.sigTime = sigTime;

            mapPurificationBroadcastTxes.insert(make_pair(txNew.GetHash(), dstx));
        }

        CInv inv(MSG_LNIX, txNew.GetHash());
        RelayInv(inv);
        ...
        // Reset
        LogPrint("purification", "CPurificationPool::Check() -- COMPLETED -- RESETTING\n");
        SetNull();
        RelayStatus(sessionID, GetState(), GetEntriesCount(), MASTERNODE_RESET);
    }
}
...
//
// Why bother? Purification uses collateral to ensure abuse to the process is kept to a minimum.
// The submission and signing stages in Purification are completely separate. In the cases where
...
//
void CPurificationPool::ChargeFees()
{
    ...
    if (state == POOL_STATUS_ACCEPTING_ENTRIES) {
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            if (!found) {
                LogPrintf("CPurificationPool::ChargeFees -- found uncooperative node (didn't send transaction). Found offence.\n");
                offences++;
            }
        }
    }
    ...
    if (state == POOL_STATUS_SIGNING) {
        ...
        BOOST_FOREACH (const CObfuScationEntry v, entries) {
            BOOST_FOREACH (const CTxDSIn s, v.sev) {
                if (!s.fHasSig) {
                    LogPrintf("CPurificationPool::ChargeFees -- found uncooperative node (didn't sign). Found offence\n");
                    offences++;
                }
            }
        }
    ...
    if (state == POOL_STATUS_ACCEPTING_ENTRIES) {
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            // This queue entry didn't send us the promised transaction
            if (!found && r > target) {
                LogPrintf("CPurificationPool::ChargeFees -- found uncooperative node (didn't send transaction). charging fees.\n");
                ...
                if (!wtxCollateral.AcceptToMemoryPool(true)) {
                    ...
                    LogPrintf("CPurificationPool::ChargeFees() : Error: Transaction not valid");
                }
        ...
        BOOST_FOREACH (const CObfuScationEntry v, entries) {
            BOOST_FOREACH (const CTxDSIn s, v.sev) {
                if (!s.fHasSig && r > target) {
                    LogPrintf("CPurificationPool::ChargeFees -- found uncooperative node (didn't sign). charging fees.\n");
...
//  - Purification is completely free, to pay miners we randomly pay the collateral of users.
void CPurificationPool::ChargeRandomFees()
{
    if (fMasterNode) {
        int i = 0;
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                Being that Purification has "no fees" we need to have some kind of cost associated
            ...
            */
            if (r <= 10) {
                LogPrintf("CPurificationPool::ChargeRandomFees -- charging random fees. %u\n", i);
                ...
                if (!wtxCollateral.AcceptToMemoryPool(true)) {
                    // This must not fail. The transaction has already been signed and recorded.
                    LogPrintf("CPurificationPool::ChargeRandomFees() : Error: Transaction not valid");
                }
                wtxCollateral.RelayWalletTransaction();
            }
        }
...
//
// Check for various timeouts (queue objects, Purification, etc)
//
void CPurificationPool::CheckTimeout()
{
    if (!fEnablePurification && !fMasterNode) return;

    // catching hanging sessions
    if (!fMasterNode) {
        switch (state) {
        case POOL_STATUS_TRANSMISSION:
            LogPrint("purification", "CPurificationPool::CheckTimeout() -- Session complete -- Running Check()\n");
            Check();
            break;
        case POOL_STATUS_ERROR:
            LogPrint("purification", "CPurificationPool::CheckTimeout() -- Pool error -- Running Check()\n");
            Check();
            break;
        case POOL_STATUS_SUCCESS:
            LogPrint("purification", "CPurificationPool::CheckTimeout() -- Pool success -- Running Check()\n");
            Check();
            break;
        }
    }

    // check Purification queue objects for timeouts
    int c = 0;
    vector<CPurificationQueue>::iterator it = vecPurificationQueue.begin();
    while (it != vecPurificationQueue.end()) {
        if ((*it).IsExpired()) {
            LogPrint("purification", "CPurificationPool::CheckTimeout() : Removing expired queue entry - %d\n", c);
            it = vecPurificationQueue.erase(it);
        } else
            ++it;
        c++;
    }
    ...
    if (state == POOL_STATUS_ACCEPTING_ENTRIES || state == POOL_STATUS_QUEUE) {
        ...
        while (it2 != entries.end()) {
            if ((*it2).IsExpired()) {
                LogPrint("purification", "CPurificationPool::CheckTimeout() : Removing expired entry - %d\n", c);
        ...
        LogPrint("purification", "CPurificationPool::CheckTimeout() -- Session timed out (%ds) -- resetting\n", OBFUSCATION_QUEUE_TIMEOUT);
        UnlockCoins();
        SetNull();
    ...
    if (state == POOL_STATUS_SIGNING && GetTimeMillis() - lastTimeChanged >= (OBFUSCATION_SIGNING_TIMEOUT * 1000) + addLagTime) {
        LogPrint("purification", "CPurificationPool::CheckTimeout() -- Session timed out (%ds) -- restting\n", OBFUSCATION_SIGNING_TIMEOUT);
        ChargeFees();
        UnlockCoins();
        SetNull();
...
//
void CPurificationPool::CheckForCompleteQueue()
{
    if (!fEnablePurification && !fMasterNode) return;
...
bool CPurificationPool::SignatureValid(const CScript& newSig, const CTxIn& newVin)
{
    CMutableTransaction txNew;
    txNew.vin.clear();
    txNew.vout.clear();
    ...
    if (found >= 0) { //might have to do this one input at a time?
        ...
        LogPrint("purification", "CPurificationPool::SignatureValid() - Sign with sig %s\n", newSig.ToString().substr(0, 24));
        if (!VerifyScript(txNew.vin[n].scriptSig, sigPubKey, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_STRICTENC, MutableTransactionSignatureChecker(&txNew, n))) {
            LogPrint("purification", "CPurificationPool::SignatureValid() - Signing - Error signing input %u\n", n);
            return false;
        }
    }

    LogPrint("purification", "CPurificationPool::SignatureValid() - Signing - Successfully validated input\n");
    return true;
}

// check to make sure the collateral provided by the client is valid
bool CPurificationPool::IsCollateralValid(const CTransaction& txCollateral)
{
    ...
    BOOST_FOREACH (const CTxOut o, txCollateral.vout) {
        ...
        if (!o.scriptPubKey.IsNormalPaymentScript()) {
            LogPrintf("CPurificationPool::IsCollateralValid - Invalid Script %s\n", txCollateral.ToString());
            return false;
        }
    }
    ...
    if (missingTx) {
        LogPrint("purification", "CPurificationPool::IsCollateralValid - Unknown inputs in collateral transaction - %s\n", txCollateral.ToString());
        return false;
    }
    ...
    if (nValueIn - nValueOut < OBFUSCATION_COLLATERAL) {
        LogPrint("purification", "CPurificationPool::IsCollateralValid - did not include enough fees in transaction %d\n%s\n", nValueOut - nValueIn, txCollateral.ToString());
        return false;
    }

    LogPrint("purification", "CPurificationPool::IsCollateralValid %s\n", txCollateral.ToString());

    {
        LOCK(cs_main);
        CValidationState state;
        if (!AcceptableInputs(mempool, state, txCollateral, true, NULL)) {
            if (fDebug) LogPrintf("CPurificationPool::IsCollateralValid - didn't pass IsAcceptable\n");
            return false;
        }
    }
...
//
bool CPurificationPool::AddEntry(const std::vector<CTxIn>& newInput, const CAmount& nAmount, const CTransaction& txCollateral, const std::vector<CTxOut>& newOutput, int& errorID)
{
    if (!fMasterNode) return false;

    BOOST_FOREACH (CTxIn in, newInput) {
        if (in.prevout.IsNull() || nAmount < 0) {
            LogPrint("purification", "CPurificationPool::AddEntry - input not valid!\n");
    ...
    if (!IsCollateralValid(txCollateral)) {
        LogPrint("purification", "CPurificationPool::AddEntry - collateral not valid!\n");
    ...
    if ((int)entries.size() >= GetMaxPoolTransactions()) {
        LogPrint("purification", "CPurificationPool::AddEntry - entries is full!\n");
    ...
    BOOST_FOREACH (CTxIn in, newInput) {
        LogPrint("purification", "looking for vin -- %s\n", in.ToString());
        BOOST_FOREACH (const CObfuScationEntry& v, entries) {
            BOOST_FOREACH (const CTxDSIn& s, v.sev) {
                if ((CTxIn)s == in) {
                    LogPrint("purification", "CPurificationPool::AddEntry - found in vin\n");
    ...
    LogPrint("purification", "CPurificationPool::AddEntry -- adding %s\n", newInput[0].ToString());
    errorID = MSG_ENTRIES_ADDED;

    return true;
}

bool CPurificationPool::AddScriptSig(const CTxIn& newVin)
{
    LogPrint("purification", "CPurificationPool::AddScriptSig -- new sig  %s\n", newVin.scriptSig.ToString().substr(0, 24));


    BOOST_FOREACH (const CObfuScationEntry& v, entries) {
        BOOST_FOREACH (const CTxDSIn& s, v.sev) {
            if (s.scriptSig == newVin.scriptSig) {
                LogPrint("purification", "CPurificationPool::AddScriptSig - already exists\n");
                return false;
            }
        }
    }
    ...
    if (!SignatureValid(newVin.scriptSig, newVin)) {
        LogPrint("purification", "CPurificationPool::AddScriptSig - Invalid Sig\n");
        return false;
    }

    LogPrint("purification", "CPurificationPool::AddScriptSig -- sig %s\n", newVin.ToString());

    BOOST_FOREACH (CTxIn& vin, finalTransaction.vin) {
        if (newVin.prevout == vin.prevout && vin.nSequence == newVin.nSequence) {
            vin.scriptSig = newVin.scriptSig;
            vin.prevPubKey = newVin.prevPubKey;
            LogPrint("purification", "CObfuScationPool::AddScriptSig -- adding to finalTransaction  %s\n", newVin.scriptSig.ToString().substr(0, 24));
        }
    }
    for (unsigned int i = 0; i < entries.size(); i++) {
        if (entries[i].AddSig(newVin)) {
            LogPrint("purification", "CObfuScationPool::AddScriptSig -- adding  %s\n", newVin.scriptSig.ToString().substr(0, 24));
            return true;
        }
    }

    LogPrintf("CPurificationPool::AddScriptSig -- Couldn't set sig!\n");
    return false;
}

// Check to make sure everything is signed
bool CPurificationPool::SignaturesComplete()
{
    BOOST_FOREACH (const CObfuScationEntry& v, entries) {
        BOOST_FOREACH (const CTxDSIn& s, v.sev) {
...
//
// Execute a Purification denomination via a Masternode.
// This is only ran from clients
//
void CPurificationPool::SendPurificationDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, CAmount amount)
{
    if (fMasterNode) {
        LogPrintf("CPurificationPool::SendPurificationDenominate() - Purification from a Masternode is not supported currently.\n");
        return;
    }

    if (txCollateral == CMutableTransaction()) {
        LogPrintf("CPurificationPool:SendPurificationDenominate() - Purification collateral not set");
        return;
    }
    ...
    if (!sessionFoundMasternode) {
        LogPrintf("CPurificationPool::SendPurificationDenominate() - No Masternode has been selected yet.\n");
        UnlockCoins();
        SetNull();
        return;
    }
    ...
    if (!CheckDiskSpace()) {
        UnlockCoins();
        SetNull();
        fEnablePurification = false;
        LogPrintf("CPurificationPool::SendPurificationDenominate() - Not enough disk space, disabling Purification.\n");
        return;
    }

    UpdateState(POOL_STATUS_ACCEPTING_ENTRIES);

    LogPrintf("CPurificationPool::SendPurificationDenominate() - Added transaction to pool.\n");
...
// Incoming message from Masternode updating the progress of Purification
//    newAccepted:  -1 mean's it'n not a "transaction accepted/not accepted" message, just a standard update
//                  0 means transaction was not accepted
//                  1 means transaction was accepted

bool CPurificationPool::StatusUpdate(int newState, int newEntriesCount, int newAccepted, int& errorID, int newSessionID)
{
    if (fMasterNode) return false;
    ...
    if (newAccepted != -1) {
        ...
        if (newAccepted == 1 && newSessionID != 0) {
            ...
            LogPrintf("CPurificationPool::StatusUpdate - set sessionID to %d\n", sessionID);
    ...
    if (newState == POOL_STATUS_ACCEPTING_ENTRIES) {
        if (newAccepted == 1) {
            LogPrintf("CPurificationPool::StatusUpdate - entry accepted! \n");
            sessionFoundMasternode = true;
            //wait for other users. Masternode will report when ready
            UpdateState(POOL_STATUS_QUEUE);
        } else if (newAccepted == 0 && sessionID == 0 && !sessionFoundMasternode) {
            LogPrintf("CPurificationPool::StatusUpdate - entry not accepted by Masternode \n");
            UnlockCoins();
            UpdateState(POOL_STATUS_ACCEPTING_ENTRIES);
...
//
bool CPurificationPool::SignFinalTransaction(CTransaction& finalTransactionNew, CNode* node)
{
    if (fMasterNode) return false;

    finalTransaction = finalTransactionNew;
    LogPrintf("CPurificationPool::SignFinalTransaction %s", finalTransaction.ToString());
        ...
        BOOST_FOREACH (const CTxDSIn s, e.sev) {
            ...
            if (mine >= 0) { //might have to do this one input at a time?
                ...
                if (foundOutputs < targetOuputs || nValue1 != nValue2) {
                    ...
                    LogPrintf("CPurificationPool::Sign - My entries are not correct! Refusing to sign. %d entries %d target. \n", foundOutputs, targetOuputs);
                    UnlockCoins();
                    SetNull();
...
void CPurificationPool::NewBlock()
{
    LogPrint("purification", "CPurificationPool::NewBlock \n");
...
// Purification transaction was completed (failed or successful)
void CPurificationPool::CompletedTransaction(bool error, int errorID)
{
    if (fMasterNode) return;
...
void CPurificationPool::ClearLastMessage()
{
    lastMessage = "";
}

//
// Passively run Purification in the background to anonymize funds based on the given configuration.
//
// This does NOT run by default for daemons, only for QT.
//
bool CPurificationPool::DoAutomaticDenominating(bool fDryRun)
{
    if (!fEnablePurification) return false;
    if (fMasterNode) return false;
    if (state == POOL_STATUS_ERROR || state == POOL_STATUS_SUCCESS) return false;
    ...
    TRY_LOCK(cs_purification, lockDS);
    if (!lockDS) {
    ...
    if (chainActive.Tip()->nHeight - cachedLastSuccess < minBlockSpacing) {
        LogPrintf("CPurificationPool::DoAutomaticDenominating - Last successful Purification action was too recent\n");
        strAutoDenomResult = _("Last successful Purification action was too recent.");
        return false;
    }

    if (mnodeman.size() == 0) {
        LogPrint("purification", "CPurificationPool::DoAutomaticDenominating - No Masternodes detected\n");
    ...
    LogPrint("purification", "DoAutomaticDenominating : nLowestDenom=%d, nBalanceNeedsAnonymized=%d\n", nLowestDenom, nBalanceNeedsAnonymized);

    // select coins that should be given to the pool
    if (!pwalletMain->SelectCoinsDark(nValueMin, nBalanceNeedsAnonymized, vCoins, nValueIn, 0, nPurificationRounds)) {
        nValueIn = 0;
        vCoins.clear();
    ...
    if (!sessionFoundMasternode) {
        ...
        if (nUseQueue > 33) {
            ...
            BOOST_FOREACH (CPurificationQueue& dsq, vecPurificationQueue) {
                ...
                if (!pwalletMain->SelectCoinsByDenominations(dsq.nDenom, nValueMin, nBalanceNeedsAnonymized, vTempCoins, vTempCoins2, nValueIn, 0, nPurificationRounds)) {
...
bool CPurificationPool::PreparePurificationDenominate()
{
    std::string strError = "";
    // Submit transaction to the pool if we get here
    // Try to use only inputs with the same number of rounds starting from lowest number of rounds possible
    for (int i = 0; i < nPurificationRounds; i++) {
        strError = pwalletMain->PreparePurificationDenominate(i, i + 1);
        LogPrintf("DoAutomaticDenominating : Running Purification denominate for %d rounds. Return '%s'\n", i, strError);
        if (strError == "") return true;
    }

    // We failed? That's strange but let's just make final attempt and try to mix everything
    strError = pwalletMain->PreparePurificationDenominate(0, nPurificationRounds);
    LogPrintf("DoAutomaticDenominating : Running Purification denominate for all rounds. Return '%s'\n", strError);
    if (strError == "") return true;
...
bool CPurificationPool::SendRandomPaymentToSelf()
{
...
bool CPurificationPool::MakeCollateralAmounts()
{
    CWalletTx wtx;
    CAmount nFeeRet = 0;
    std::string strFail = "";
...
bool CPurificationPool::CreateDenominated(CAmount nTotalValue)
{
    CWalletTx wtx;
    CAmount nFeeRet = 0;
    std::string strFail = "";
...
bool CPurificationPool::IsCompatibleWithEntries(std::vector<CTxOut>& vout)
{
...
bool CPurificationPool::IsCompatibleWithSession(int64_t nDenom, CTransaction txCollateral, int& errorID)
{
    if (nDenom == 0) return false;

    LogPrintf("CPurificationPool::IsCompatibleWithSession - sessionDenom %d sessionUsers %d\n", sessionDenom, sessionUsers);

    if (!unitTest && !IsCollateralValid(txCollateral)) {
        LogPrint("purification", "CPurificationPool::IsCompatibleWithSession - collateral not valid!\n");
        errorID = ERR_INVALID_COLLATERAL;
        return false;
    }
    ...
    if ((state != POOL_STATUS_ACCEPTING_ENTRIES && state != POOL_STATUS_QUEUE) || sessionUsers >= GetMaxPoolTransactions()) {
        ...
        LogPrintf("CPurificationPool::IsCompatibleWithSession - incompatible mode, return false %d %d\n", state != POOL_STATUS_ACCEPTING_ENTRIES, sessionUsers >= GetMaxPoolTransactions());
        return false;
    }
...
void CPurificationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
...
int CPurificationPool::GetDenominations(const std::vector<CTxDSOut>& vout)
{
...
int CPurificationPool::GetDenominations(const std::vector<CTxOut>& vout, bool fSingleRandomDenom)
{
...
int CPurificationPool::GetDenominationsByAmounts(std::vector<CAmount>& vecAmount)
{
    CScript e = CScript();
...
int CPurificationPool::GetDenominationsByAmount(CAmount nAmount, int nDenomTarget)
{
    CScript e = CScript();
...
std::string CPurificationPool::GetMessageByID(int messageID)
{
    switch (messageID) {
    ...
    case ERR_MAXIMUM:
        return _("Value more than Purification pool maximum allows.");
    case ERR_MN_LIST:
    ...
    case ERR_RECENT:
        return _("Last Purification was too recent.");
    case ERR_SESSION:
...
bool CPurificationQueue::Sign()
{
    ...
    if (!obfuScationSigner.SetKey(strMasterNodePrivKey, errorMessage, key2, pubkey2)) {
        LogPrintf("CPurificationQueue():Relay - ERROR: Invalid Masternodeprivkey: '%s'\n", errorMessage);
        return false;
    }

    if (!obfuScationSigner.SignMessage(strMessage, errorMessage, vchSig, key2)) {
        LogPrintf("CPurificationQueue():Relay - Sign message failed");
        return false;
    }

    if (!obfuScationSigner.VerifyMessage(pubkey2, vchSig, strMessage, errorMessage)) {
        LogPrintf("CPurificationQueue():Relay - Verify message failed");
        return false;
    }
...
bool CPurificationQueue::CheckSignature()
{
    ...
    if (pmn != NULL) {
        ...
        if (!obfuScationSigner.VerifyMessage(pmn->pubKeyMasternode, vchSig, strMessage, errorMessage)) {
            return error("CPurificationQueue::CheckSignature() - Got bad Masternode address signature %s \n", vin.ToString().c_str());
        }
...
void CPurificationPool::RelayFinalTransaction(const int sessionID, const CTransaction& txNew)
{
    LOCK(cs_vNodes);
    BOOST_FOREACH (CNode* pnode, vNodes) {
...
void CPurificationPool::RelayIn(const std::vector<CTxDSIn>& vin, const int64_t& nAmount, const CTransaction& txCollateral, const std::vector<CTxDSOut>& vout)
{
    if (!pSubmittedToMasternode) return;
...
void CPurificationPool::RelayStatus(const int sessionID, const int newState, const int newEntriesCount, const int newAccepted, const int errorID)
{
    LOCK(cs_vNodes);
    BOOST_FOREACH (CNode* pnode, vNodes)
...
void CPurificationPool::RelayCompletedTransaction(const int sessionID, const bool error, const int errorID)
{
    LOCK(cs_vNodes);
    BOOST_FOREACH (CNode* pnode, vNodes)
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("loonie-purification");
