                                                    ...
                                                    fFeeMinimized(true)
{
    ...
    if (!settings.contains("bUseBriskTX"))
        settings.setValue("bUseBriskTX", false);

    bool useObfuScation = settings.value("bUseObfuScation").toBool();
    bool useBriskTX = settings.value("bUseBriskTX").toBool();
    if (fLiteMode) {
        ui->checkUsePurification->setChecked(false);
        ui->checkUsePurification->setVisible(false);
        ui->checkBriskTX->setVisible(false);
        CoinControlDialog::coinControl->useObfuScation = false;
        CoinControlDialog::coinControl->useBriskTX = false;
    } else {
        ui->checkUsePurification->setChecked(useObfuScation);
        ui->checkBriskTX->setChecked(useBriskTX);
        CoinControlDialog::coinControl->useObfuScation = useObfuScation;
        CoinControlDialog::coinControl->useBriskTX = useBriskTX;
    }

    connect(ui->checkUsePurification, SIGNAL(stateChanged(int)), this, SLOT(updateDisplayUnit()));
    connect(ui->checkBriskTX, SIGNAL(stateChanged(int)), this, SLOT(updateBriskTX()));
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    if (ui->checkUsePurification->isChecked()) {
        recipients[0].inputType = ONLY_DENOMINATED;
        ...
        strFee = QString(tr(
            "(purification requires this amount to be rounded up to the nearest %1).")
    ...
    if (ui->checkBriskTX->isChecked()) {
        recipients[0].useBriskTX = true;
        strFunds += " ";
        strFunds += tr("and BriskTX");
    } else {
        recipients[0].useBriskTX = false;
    }
...
void SendCoinsDialog::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& anonymizedBalance, const CAmount& watchBalance, const CAmount& watchUnconfirmedBalance, const CAmount& watchImmatureBalance)
{
    ...
    if (model && model->getOptionsModel()) {
        uint64_t bal = 0;
        QSettings settings;
        settings.setValue("bUseObfuScation", ui->checkUsePurification->isChecked());
        if (ui->checkUsePurification->isChecked()) {
...
void SendCoinsDialog::updateDisplayUnit()
{
    ...
    setBalance(model->getBalance(), model->getUnconfirmedBalance(), model->getImmatureBalance(), model->getAnonymizedBalance(),
        model->getWatchBalance(), model->getWatchUnconfirmedBalance(), model->getWatchImmatureBalance());
    CoinControlDialog::coinControl->useObfuScation = ui->checkUsePurification->isChecked();
    coinControlUpdateLabels();
...
void SendCoinsDialog::updateBriskTX()
{
    QSettings settings;
    settings.setValue("bUseBriskTX", ui->checkBriskTX->isChecked());
    CoinControlDialog::coinControl->useBriskTX = ui->checkBriskTX->isChecked();
    coinControlUpdateLabels();
}
...
void SendCoinsDialog::coinControlUpdateLabels()
{
    ...
    ui->checkUsePurification->setChecked(CoinControlDialog::coinControl->useObfuScation);
